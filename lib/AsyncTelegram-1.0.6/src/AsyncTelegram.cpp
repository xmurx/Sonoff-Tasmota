...
// uint8_t fingerprint[20] = { 0xB2, 0x72, 0x47, 0xA6, 0x69, 0x8C, 0x3C, 0x69, 0xF9, 0x58, 0x6C, 0xF3, 0x60, 0x02, 0xFB, 0x83, 0xFA, 0x8B, 0x1F, 0x23 };


AsyncTelegram::AsyncTelegram()
: telegramClient(tls_rx_size, tls_tx_size)
{
    setFingerprint(fingerprint);   // set the default fingerprint   
#if defined(ESP8266) 
    telegramClient.setPubKeyFingerprint(m_fingerprint, m_fingerprint, false );   
...
bool AsyncTelegram::getUpdates(){   
    ...
    if(millis() - m_lastUpdateTime > m_minUpdateTime){
        ...
        if( httpData.waitingReply == false) {   
            String param((char *)0);
            param.reserve(64);
            StaticJsonBuffer<BUFFER_SMALL> buffer;
...
MessageType AsyncTelegram::getNewMessage(TBMessage &message ) 
{
    ...
    if( httpData.payload.length() > 0 ) {       
        ...
        DynamicJsonBuffer buffer(BUFFER_BIG);
        JsonObject& root = buffer.parseObject(httpData.payload);
        ...
        if(root["result"][0]["callback_query"]["id"].success()){
        ...
        else if(root["result"][0]["message"]["message_id"].success()){
            // this is a message
            ...
            if(root["result"][0]["message"]["location"].success()){
            ...
            else if(root["result"][0]["message"]["contact"].success()){
            ...
            else if (root["result"][0]["message"]["text"].success()) {
...
#endif
    int ret = telegramClient.connect(TELEGRAM_HOST, TELEGRAM_PORT);
    xControl::Logging::Debug("client.connect() ret: %d", ret);
    return getMe(m_user);
}
...
bool AsyncTelegram::getMe(TBUser &user) {
    ...
    DynamicJsonBuffer buffer(BUFFER_SMALL);
    JsonObject& root = buffer.parseObject(response);
...
void AsyncTelegram::sendMessage(const TBMessage &msg, const char* message, String keyboard)
{
    ...
    if (keyboard.length() != 0) {
        ...
        JsonObject& myKeyb = buffer.parseObject(keyboard);
...
void AsyncTelegram::sendPhotoByUrl(const uint32_t& chat_id,  const String& url, const String& caption){ 
    ...
    StaticJsonBuffer<BUFFER_BIG> buffer;
    JsonObject& root = buffer.createObject();   
...
bool AsyncTelegram::serverReply(const char* const& replyMsg) {  
    StaticJsonBuffer<BUFFER_SMALL> buffer; 
    JsonObject& root = buffer.parseObject(replyMsg);
    bool ok = root["ok"];
    if (!ok) {
#if DEBUG_MODE > 0
        ...
        if (m_useDNS) {
            ...
            if (!telegramClient.connect(TELEGRAM_HOST, TELEGRAM_PORT)) {
                ...
                if (!telegramClient.connect(telegramServerIP, TELEGRAM_PORT)) {
                    xControl::Logging::Debug("\n__Unable to connect to Telegram server\n");                  
                }
                else {
